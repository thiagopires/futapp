...
def load_daymatches(dt, source):
    try:
        ...
        if source == 'Betfair':
            ...
            df["Odd_DC_1X"] = round((df["Odd_H_FT"] * df["Odd_D_FT"]) / (df["Odd_H_FT"] + df["Odd_D_FT"] - 1),2)
            df["Odd_DC_12"] = round((df["Odd_H_FT"] * df["Odd_A_FT"]) / (df["Odd_H_FT"] + df["Odd_A_FT"] - 1),2)
            df["Odd_DC_X2"] = round((df["Odd_D_FT"] * df["Odd_A_FT"]) / (df["Odd_D_FT"] + df["Odd_A_FT"] - 1),2)
        elif source == 'FootyStats':
