...
def set_odds_filtros(reset=False):
    if reset:
        ...
        if "odd_h_min" not in st.session_state: st.session_state['odd_h_min'] = 4.00
        if "odd_h_max" not in st.session_state: st.session_state['odd_h_max'] = 50.00
        ...
        if "odd_a_min" not in st.session_state: st.session_state['odd_a_min'] = 1.40
        if "odd_a_max" not in st.session_state: st.session_state['odd_a_max'] = 2.00
...
#     (df_hist['Away'] == visitante) & 
#     (df_hist['Resultado_FT'].str.replace("-","x") == placar) &
#     (df_hist['Season'] == get_current_season()) # verificar
# ]
